...
if (!empty($_POST["featured"]))
	$epicParams[] = "starFeatured = 1";
if (!empty($_POST["epic"]))
	$epicParams[] = "starEpic = 1";
if (!empty($_POST["mythic"]))
	$epicParams[] = "starEpic = 2";
if (!empty($_POST["legendary"]))
	$epicParams[] = "starEpic = 3";
...
foreach ($result as &$level1) {
	if ($level1["levelID"] != "") {
		if ($isIDSearch and $level1['unlisted'] > 1) {
			if (!isset($accountID))
				$accountID = GJPCheck::getAccountIDOrDie();
			if (!$gs->isFriends($accountID, $level1['extID']) && $accountID != $level1['extID'])
				break;
		}
